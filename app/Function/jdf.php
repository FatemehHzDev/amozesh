...
/*	F	*/
function calc($x1,$x2,$x3,$x4,$x5,$x6,$x7,$x8,$x9,$x10)
{
    $x1_y=substr($x1, 0, 4);
    $x1_m=substr($x1, 5, 2);
    $x1_d=substr($x1, 8, 3);
    ...
    $t_b= jmktime($h, $m, 0, $x1_m, $x1_d, $x1_y);
    $t_s= jmktime($hs, $ms, 0, $x1_m, $x1_d, $x1_y);
    $t_e= jmktime($he, $me, 0, $x1_m, $x1_d, $x1_y);

    $x1_t=jmktime(0, 0, 0, $x1_m, $x1_d, $x1_y);
    $s_t=jmktime(0, 0, 0, $x5, $x6, $x7);
    $e_t=jmktime(0, 0, 0, $x8, $x9, $x10);

    if($x1_t >= $s_t && $x1_t <= $e_t  && $t_b >= $t_s && $t_b <= $t_e )
    {
       return true;
    }
    else
...
{
    $dy1 = substr($date1, 0, 8);
    $dm1 = substr($date1, 9, 4);
    $dd1 = substr($date1, 14, 4);
    $time1= jmktime('0', '0', '0', $dm1, $dd1, $dy1);
    $dy2 = substr($date2, 0, 8);
    $dm2 = substr($date2, 9, 4);
    $dd2 = substr($date2, 14, 4);
    $time2= jmktime('0', '0', '0', $dm2, $dd2, $dy2);
    if($date1 >=$date2){
        return true;
    }
    else{
